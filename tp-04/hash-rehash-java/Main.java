...
class Hash 
{
    ...
    //constructor
    public Hash(int len) throws Exception{
        if(len >= 0) {
            acessArea = len;
            hashTable = new Personagem[acessArea];
            for (int i = 0; i < acessArea; i++) { hashTable[i] = null; }
        
        } else { throw new Exception("ERROR VALUE INVALID."); }
    }
    
    private int getHashCode(int persLen)    { return (persLen % 21);  }
    private int getReHashCode(int persLen)  { return (persLen % 25 ); }
    ...
    public boolean insert(Personagem pers) {
        ...
        if( hashTable[hashCode] == null ) { hashTable[hashCode] = pers; resp = true; }
            ...
            hashCode = getReHashCode(pers.getAltura() + 1);
            if( hashTable[hashCode] == null ) { hashTable[hashCode] = pers; resp = true; }
            else {
                return false;
            }
