...
int numSwapsRadixSort    = 0;
int numCompsRadixSort = 0;
...
bool compareStr(Personagem p1, Personagem p2) {
    bool resp = false;
    if     (   strcmp(p1.anoNascimento, p2.anoNascimento) < 0)  resp = true;
    else if(  strcmp(p1.anoNascimento, p2.anoNascimento) == 0 ) resp = ( strcmp(p1.nome , p2.nome ) < 0);
    return resp;
}

//--------------------------radixSort-----------------------------------//

// Retorna o dígito na posição "pos" do número "num"
...
// Ordena o array "arr" de tamanho "n" baseado no dígito na posição "pos"
void countingSort(Lista *list, int n, int pos) {
    int count[10] = {0};
    Personagem output[n];

    // Conta a frequência de cada dígito na posição "pos"
    for (int i = 0; i < n; i++) {
        int digit = getDigit(list->array[i].altura, pos);
        count[digit]++;
    }
    ...
    for (int i = n - 1; i >= 0; i--) {
        int digit = getDigit(list->array[i].altura, pos);
        output[count[digit] - 1] = list->array[i];
        count[digit]--;
    }

    // Copia o array de saída para o array original
    for (int i = 0; i < n; i++) {
        list->array[i] = output[i];
    }
}

// Ordena o array "arr" de tamanho "n" baseado na altura dos elementos
void radixSort(Lista *list, int n) {
    int max = list->array[0].altura;
    for (int i = 1; i < n; i++) {
        if (list->array[i].altura > max) {
            max = list->array[i].altura;
            numCompsRadixSort++;
        }
    }

    // Realiza o counting sort para cada dígito do maior elemento
    for (int pos = 1; max / pos > 0; pos *= 10) {
        countingSort(list, list->length, pos);
    }
}
...
//--------------------------------------------------------------------------//
    ...
    radixSort(listaPers, listaPers->length);
    
    printList(listaPers);

    FILE *f = fopen("748189_radixsort.txt", "wt");
    fprintf(f, "749190\t %ld\t %d", (clock() - startTime), numCompsRadixSort);
