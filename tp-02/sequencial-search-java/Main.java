...
class Main 
{
    ...
    public static void main(String[] args) {
        ...
        String procurarNome = MyIO.readLine();
        
        //leitura de valolres a ser procurado
        while (isFim(procurarNome) == false) { 
            //condicao de parada
            
            // charList.array[j].getNome() == procurarNome
            boolean encontrado = false;
            for (int j = 0; j < charList.n && encontrado == false; j++) {
                numComp++;
                if(charList.array[j].getNome().equals(procurarNome)) { encontrado = true; }
            }
            MyIO.println(encontrado==true ?"SIM":"NAO");
            
            procurarNome = MyIO.readLine();
        
        } 

        Arq.openWrite("748189_sequencial.txt");
