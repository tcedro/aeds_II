...
int numSwapShellSort = 0;
int numCompShellSort = 0;
...
bool compareMass(Personagem p1, Personagem p2) {
    bool resp = false;
    // printf("nome1: %s [peso1]:%lf nome2: %s[peso2]%lf\n",p1.nome, p1.peso, p2.nome, p2.peso);
    if ( p1.peso < p2.peso ) { resp = true; }
    else if( p1.peso == p2.peso ) { resp = ( strcmp(p1.nome, p2.nome ) < 0); }
    // printf("[resp]:%d\n", resp);
    return resp;
}
...
void insertionByColor(Lista *charList, int cor, int h){
    for (int i = (h + cor); i < charList->length; i+=h) {
        Personagem tmp = clone(tmp, charList->array[i]);
        int j = i - h;
        while ((j >= 0)  && compareMass(charList->array[j], tmp) == false ) {
            numCompShellSort++;
            charList->array[j + h] = charList->array[j];
            j-=h;
        }
        numSwapShellSort++;
        charList->array[j + h] = tmp;
    }
}

//-------------------------------shellsort---------------------------------------//

void shellsort(Lista *charList) {
    int h = 1;

    do { h = (h * 3) + 1; } while (h < charList->length);

    do {
        h /= 3;
        for(int cor = 0; cor < h; cor++){
            insertionByColor(charList, cor, h);
        }
    } while (h != 1);
}
...
//--------------------------------------------------------------------------//
    ...
    FILE *f = fopen("748189_selecaoRecursivo.txt", "wt");
    fprintf(f, "749190\t %ld\t %d", (clock() - startTime), numCompShellSort);
