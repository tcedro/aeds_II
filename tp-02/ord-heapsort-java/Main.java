...
class CharacterList  
{
    ...
    public boolean compare(Personagem p1, Personagem p2) {
        boolean resp = false;
        
        if ( p1.getHomeWorld().compareTo(p2.getHomeWorld()) < 0 ) { resp = true; }
        
        else if( p1.getHomeWorld().compareTo(p2.getHomeWorld()) == 0 ) {
            resp = ( p1.getNome().compareTo( p2.getNome() ) < 0);
            
        }
        return resp;
    }

    public void mergeSort(Personagem[] psArray, int n) {
        if (n < 2) return;

        int mid = n / 2;

        Personagem[] l_array = new Personagem[mid];
        Personagem[] r_array = new Personagem[n - mid];

        for (int i = 0; i < mid; i++) l_array[i] = psArray[i];
        for (int i = mid; i < n; i++) r_array[i-mid] = psArray[i];

        mergeSort(l_array, mid);
        mergeSort(r_array, n - mid);

        merge(psArray, l_array, r_array, mid, n - mid);
    }

    public void merge(Personagem[] psArray, Personagem[] l_array,Personagem[] r_array, int left, int right)  {
        int i = 0, j = 0, k = 0;
        while (i < left && k < right) {
            if ( compare( l_array[i], r_array[j] ) == true)  psArray[k++] = l_array[i++];
            else psArray[k++] = r_array[j++];
            
        }

        while (i < left)  psArray[k++] = l_array[i++];
        while (j < right) psArray[k++] = r_array[j++];
    ...
    public void heapSort() {
        Personagem[] tmp = new Personagem[this.n+1];
        for ( int i = 0; i < this.n; i++ ) tmp[i+1] = this.array[i];
        this.array = tmp;

        //construir heap
        for (int tamHeap = 2; tamHeap < this.n; tamHeap++) {
            constructor(tamHeap);
        }
        ...
        while (tamHeap > 1) {
            swap(1, tamHeap--);
            reconstructor(tamHeap);
        }

        //altera o vetor
        tmp  = array;
        array = new Personagem [this.n];
        for (int i = 0; i < n; i++) array[i] = tmp[i+1];
    }

    public void constructor(int tamHeap) {
        for (int i = tamHeap; i > 1 && this.array[i].getAltura() > this.array[i/2].getAltura(); i /= 2) {
            swap(i, i/2);
        }
    }

    public void reconstructor(int tamHeap) {
        int i = 1;
        
        while (i <= (tamHeap/2)) {
            int son = getBiggerSon(i, tamHeap);
            
            if (this.array[i].getAltura() < this.array[son].getAltura()) {
                swap(i, son);
                i = son;
    ...
    public int getBiggerSon(int i, int tamHeap) {
        int son; 
        
        if (2*i == tamHeap || this.array[2*i].getAltura() > this.array[2*i+1].getAltura()) {
            son = 2 * i;
...
class Main 
{
    ...
    public static void main(String[] args) {
        ...
        Arq.openWrite("748189_heapsort.txt");
