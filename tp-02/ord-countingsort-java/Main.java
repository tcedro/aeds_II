...
class CharacterList  
{
    ...
    public Integer getUpper() {
        Integer upper = (int)this.array[0].getPeso();
        for (int i = 0; i < this.n; i++) {
            if(upper < array[i].getPeso()) {
                upper = (int)array[i].getPeso();
            }
        }
        return upper;
    }
    ...
    public int countingSort() {
        int tamCount = getUpper() + 1;
        int[] count = new int[tamCount];
        int numCompares = 0;
        Personagem[] order = new Personagem[this.n];

        for(int i = 0; i < tamCount; count[i] = 0, i++) numCompares++;
        for(int i = 0; i < n; count[(int)array[i].getPeso()]++, i++)numCompares++;
        for(int i = 1; i < tamCount; count[i] += count[i-1], i++)numCompares++;
        for(int i = n-1; i >= 0; order[count[ (int)array[i].getPeso()]-1] = array[i], count[(int)array[i].getPeso()]--, i--)numCompares++;
        for(int i = 0; i < n; array[i] = order[i], i++)numCompares++;
...
class Main 
{
    ...
    public static void main(String[] args) {
        ...
        numComp = charList.countingSort();

        for (int i = 0; i < charList.n; i++) {
            MyIO.println(charList.array[i].toString() );
        } 

        Arq.openWrite("748189_coutingsort.txt");
