...
int numSwapsRecSelection = 0;
int numCompsRecSelection = 0;
...
// void quickSort(int esq, int dir, Lista *listaPers) {
//         int i = esq, j = dir;
...
//             }
//         }
//     if(esq < j) quickSort(esq, j, listaPers);
//     if(i < dir) quickSort(i, dir, listaPers);

// }

//---------------------------bubbleSort---------------------------------//

// void bubbleSort(Lista *charList) {
//     for (int i = (charList->length - 1); i > 0; i--) {
//         for (int j = 0; j < i; j++) {
//             numCompsRecSelection++;
//             if (compareStr(charList->array[j+1], charList->array[j])) {
//                 numSwapsRecSelection++;
//                 swap(j, j+1, charList);
//             }
//         }
//     }
...
void r_selectionSort(int i, Lista *charList) {
    int lower = i;
    for(int j = (i + 1); i < charList->length; i++) {
        if ( strcmp(charList->array[lower].nome, charList->array[j].nome) > 0 )  {
            lower = j;
        }
        numCompsRecSelection++;        
    }
    
    swap(lower, i, charList);
        numSwapsRecSelection++;

    if (i + 1 < charList->length - 1) {
        r_selectionSort(i + 1, charList);
    }
}
...
//--------------------------------------------------------------------------//
    ...
    r_selectionSort(listaPers->length , listaPers);
    printList(listaPers);

    FILE *f = fopen("748189_selecaoRecursivo.txt", "wt");
    fprintf(f, "749190\t %ld\t %d", (clock() - startTime), numCompsRecSelection);
