    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numOp; i++) {
            ...
            //------------------------------ Inserir ----------------------------------//
                ...
                //personagem é inserido a lista
                charList.inserirMeio(pos, pers);
            }
            
            //------------------------------ Remover no inicio ----------------------------------//

            else if( func.compareTo("RI") == 0) { MyIO.println( "(R) " + charList.removerInicio().getNome() ); }

            //------------------------------ Remover no fim ----------------------------------//

            else if( func.compareTo("RF") == 0) { MyIO.println( "(R) " + charList.removerFim().getNome() ); }

            //----------------------------------- Remover ----------------------------------//

            else if( func.compareTo("R*") == 0) {
                Integer pos = Integer.parseInt(tmp.substring(3, tmp.length()));
                MyIO.println( "(R) " + charList.removerMeio(pos).getNome() );
            }
        }

        // PRINT LIST
        MyIO.print(charList.toString());
    }
}